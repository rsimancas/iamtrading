...
{
    public static class EstatusDistribucion
    {
        static EstatusDistribucion()
        {
        ...
        private static int? getEstatus(string estatusName)
        {
            ...
            {
                LogManager.Write("ERROR:" + Environment.NewLine + "\tMETHOD = EstatusDistribucion." + MethodBase.GetCurrentMethod().Name + Environment.NewLine + "\tMESSAGE = " + ex.Message);
                throw;
            };


            string sql = "select EstatusId " +
                         " from Estatus where EstatusNombre = '{0}' and EstatusTipo='D'";
